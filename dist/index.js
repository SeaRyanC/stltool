...
function isVec3String(s) {
    return /^-?[\d.]+,-?[\d.]+,-?[\d.]+$/.test(s);
}
function isAbout(s) {
    return s === 'origin' || s === 'center' || isVec3String(s);
}
function resolveAbout(s, mesh) {
    if (s === 'origin')
        return { x: 0, y: 0, z: 0 };
    if (s === 'center')
...
function getOutputPath(inputFile, output, outDir, overwrite) {
    if (overwrite)
        return inputFile;
    if (output) {
...
async function applyOp(mesh, op) {
    switch (op.type) {
        case 'center':
        ...
        case 'fdm': {
            ...
            if (!valid)
                console.warn('Warning: mesh is not manifold');
