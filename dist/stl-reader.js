...
function parseBinaryStl(buf) {
    ...
    for (let i = 0; i < count; i++) {
        const normal = readVec3(buf, offset);
        offset += 12;
        const v1 = readVec3(buf, offset);
        offset += 12;
        const v2 = readVec3(buf, offset);
        offset += 12;
        const v3 = readVec3(buf, offset);
        offset += 12;
        offset += 2; // attribute
...
function parseAsciiStl(content) {
    ...
    while (i < lines.length) {
        if (lines[i].startsWith('facet normal')) {
            ...
            const normal = { x: parseFloat(parts[2]), y: parseFloat(parts[3]), z: parseFloat(parts[4]) };
            i++; // outer loop
            i++;
            const verts = [];
            for (let j = 0; j < 3; j++, i++) {
...
export async function readStlFile(filePath) {
