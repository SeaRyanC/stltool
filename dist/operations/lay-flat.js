import { crossVec3, dotVec3, normalizeVec3, magnitudeVec3, scaleVec3, addVec3, computeNormal, triangleArea } from '../geometry.js';
...
function rotateMeshByAxisAngle(mesh, k, theta) {
    const rotVert = (v) => rodrigues(v, k, theta);
    return {
...
export function layFlat(mesh, axis = 'z', verbose = false) {
        ...
        if (overhangArea < bestScore) {
            ...
            if (rotAxisMag < 1e-10) {
                ...
                if (dp < 0) {
                    ...
                    const perp = Math.abs(downDir.x) < 0.9 ? { x: 1, y: 0, z: 0 } : { x: 0, y: 1, z: 0 };
