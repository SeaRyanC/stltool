...
export function rotateMesh(mesh: Mesh, axis: 'x' | 'y' | 'z', degrees: number, about: Vec3 = { x: 0, y: 0, z: 0 }): Mesh {
  const theta = (degrees * Math.PI) / 180;
  const k: Vec3 = axis === 'x' ? { x: 1, y: 0, z: 0 } : axis === 'y' ? { x: 0, y: 1, z: 0 } : { x: 0, y: 0, z: 1 };
  const rotVert = (v: Vec3): Vec3 => addVec3(about, rodrigues(subVec3(v, about), k, theta));
  return {
